...
void main() => runApp(
    const MaterialApp(debugShowCheckedModeBanner: false, home: CachedBlurhashApp()));

class CachedBlurhashApp extends StatefulWidget {
  const CachedBlurhashApp({super.key});

  @override
  CachedBlurhashAppState createState() => CachedBlurhashAppState();
}

class CachedBlurhashAppState extends State<CachedBlurhashApp> {
  ...
  Container buildEntry(bool isInView, int idx) => Container(
      ...
      child: isInView || idx == 0
          ? SynchronizedDisplay(
              ...
              uri: entries[idx][1],
              title: entries[idx][2])
          : CachedBlurhash(hash: entries[idx][0]));
}
